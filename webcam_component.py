...
try:
    from violation_dashboard import render_live_violation_panel
    _VDASH = True
except ImportError:
    _VDASH = False
import cv2
...
class WebcamPPEDetector:
    ...
    def __init__(self, detection_engine: PPEDetectionEngine, attendance_manager=None):
        ...
        self.latest_stats_cache = None
    
        self.current_frame_stats = None  # most recent frame stats
        ...
        if self.detection_enabled and self.detection_engine:
            try:
                ...
                if 'error' not in results:
                    ...
                    # Draw on RGB image with per-person violation tags
                    img_rgb = self.detection_engine.draw_detections(
                        img_rgb,
                        results['detections'],
                    ...
                    # Convert back to BGR for WebRTC display
                    img = cv2.cvtColor(img_rgb, cv2.COLOR_RGB2BGR)
                    # Extract compliance_stats for stats update below
                    compliance_stats = results['compliance_stats']
                    
                    # Update statistics
                    # compliance_stats already used in draw_detections above
                    ...
                    current_stats = {
                        ...
                        'current_violations_detail': results['compliance_stats'].get('violations', []),
                        'current_people_detail': results['compliance_stats'].get('total_people', 0),
                    }

                    self.current_frame_stats = current_stats  # immediate access
                    # Add to queue (non-blocking)
                    self.latest_stats_cache = current_stats
                    try:
                ...
                import logging
                logging.error(f"video_frame_callback error: {e}", exc_info=True)
            ...
            if self.frame_count % 10 == 0:  # Update every 10 frames to avoid spam
                basic_stats = {
                    ...
                    'total_people': 0,
                    'violations': 0,
                    'compliance_rate': 0.0,  # 0% when detection off
                    'session_duration': time.time() - self.start_time,
    ...
    def get_latest_stats(self) -> Dict[str, Any]:
        ...
        # Return the most recent frame stats immediately if available
        if hasattr(self, "current_frame_stats") and self.current_frame_stats:
            # Drain queue to keep it fresh
            while not self.stats_queue.empty():
                try: self.stats_queue.get_nowait()
                except: break
            return self.current_frame_stats
        ...
        if latest_stats is not None:
            self.latest_stats_cache = latest_stats
            return latest_stats

        if self.latest_stats_cache is not None:
            return self.latest_stats_cache
        ...
        if latest_stats is None and self.frame_count > 0:
            ...
            latest_stats = {
                'frame_count': self.frame_count,
                'total_people': self.people_history[-1] if self.people_history else 0,
                'violations': self.violation_history[-1] if self.violation_history else 0,
                'compliance_rate': self.compliance_history[-1] if self.compliance_history else 100.0,
                ...
                'avg_people': sum(self.people_history) / len(self.people_history) if self.people_history else 0.0,
                'avg_violations': sum(self.violation_history) / len(self.violation_history) if self.violation_history else 0.0
            }
            self.latest_stats_cache = latest_stats
            ...
            with stats_container:
                if latest_stats and latest_stats['frame_count'] > 0:
                    ...
                    if st.session_state.analytics_loading and not st.session_state.analytics_ready:
                        ...
                        # Status indicator with NON-COMPLIANT priority
                        st.markdown("---")
                        if violations > 0:
                            st.error(f"\U0001f6a8 **NON-COMPLIANT** \u2014 {violations} violation(s) detected!")
                            if _VDASH:
                                cs_live = {'violations': latest_stats.get('current_violations_detail', []), 'total_people': latest_stats.get('total_people', 0)}
                                render_live_violation_panel(cs_live, latest_stats.get('frame_count', 0))
                        elif compliance_rate >= 90 and people_count > 0:
                            st.success("\U0001f7e2 **EXCELLENT** - All PPE requirements met!")
